...
class IncomeDataSourceTest {
  ...
  void 収入を登録して取得できる() {
    ...
    Income result = sut.get(income.incomeIdentifier());
    ...
    assertEquals(
        ...
        result.incomeAttribute().incomeAttributeIdentifier());
  }

  @Test
  void 存在しないIDで取得すると例外が発生する() {
    IncomeIdentifier unknownId = new IncomeIdentifier(UUID.randomUUID().toString());
    assertThrows(IncomeNotFoundException.class, () -> sut.get(unknownId));
  }
  ...
  void 属性で収入を検索できる() {
    ...
    Incomes result = sut.find(attribute);
    int count = 0;
    for (Income i : result) {
      count++;
    }
  ...
  void 収入を更新できる() {
    ...
    Income fetched = sut.get(income.incomeIdentifier());
    Income updated =
        new Income(
